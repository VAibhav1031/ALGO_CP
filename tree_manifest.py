        ...
        if left.lower() == "y":
            ...
            while True:
                try:
                    value = int(
                        input(
                            f"Enter the value for the left node of the {node.value} >> "
                        )
                    )
                    break
        ...
        if right.lower() == "y":
            ...
            while True:
                try:
                    value = int(
                        input(
                            f"Enter the value for the right node of the {
                                node.value
                            } >> "
                        )
                    )
...
class BST(BinaryTree):
    ...
    def _height(
        self, node
    ):  # this  function only helps  in returning the height of the node, it doesnt calculate, cause it is already calculated in the node itself
        if node is None:
            return -1
        return node.height
    ...
    def _insert(self, value, node):
        # it is  something  when we hit the base null value then we create a new node and return it  and it will save to desired location(left or right)
        if node is None:
            new_node = Node(value)
            return new_node
        ...
        # if  everything goes right we are returning the node , at which changes were ocurred
        balanced = self.self_balancing(node)
        if balanced is None:
            raise Exception("self_balancing returned None — tree structure broken.")
        return balanced
    ...
    def _isbalanced(self, node):
        ...
        return (
            abs(self.height_bst(node.left) - self.height_bst(node.right)) <= 1
            and self._isbalanced(node.left)
            and self._isbalanced(node.right)
        )
    ...
    # Self balancing AVL part of the BST class
    ...
    def self_balancing(self, node):
        ...
        if height_diff > 1:
            # left heavy
            if self.height_bst(node.left.left) >= self.height_bst(node.left.right):
                # left-left heavy
        ...
        if height_diff < -1:
            # Right heavy
            if self.height_bst(node.right.right) >= self.height_bst(node.right.left):
                # right-right heavy
